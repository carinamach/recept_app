export default function Input({ label, id, error, className = '', ...rest }) {
  ...
  return (
    <label className={`block ${className}`} htmlFor={inputId}>
      ...
      <input
        id={inputId}
        className={`w-full rounded-2xl border border-ink-300 bg-parchment-50 px-4 py-3 text-sm text-ink-900 shadow-sm outline-none transition-[box-shadow,transform,border-color] placeholder:text-ink-600 hover:border-accent/40 focus-visible:border-accent focus-visible:shadow-[0_0_0_3px_rgb(129_178_154/0.35)] dark:border-ink-600 dark:bg-ink-800 dark:text-parchment-50 dark:placeholder:text-parchment-400 dark:focus-visible:border-accent`}
        {...rest}
      />
      {error && (
